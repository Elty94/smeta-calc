          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              <div className="bg-white rounded-xl shadow-sm border p-6">
                ...
                <p className="text-2xl font-bold text-gray-900">{fmt(result.totalEstimate)}</p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border p-6">
                <p className="text-sm text-gray-500">Себестоимость работ</p>
                <p className="text-2xl font-bold text-gray-900">{fmt(result.totalContractorCost)}</p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border p-6">
                <p className="text-sm text-gray-500">Прибыль</p>
                <p className={`text-2xl font-bold ${pColor(result.profitabilityPct)}`}>{fmt(result.totalProfit)}</p>
              </div>
              <div className={`rounded-xl shadow-sm border p-6 ${pBg(result.profitabilityPct)}`}>
                <p className="text-sm text-gray-500">Рентабельность</p>
                <p className={`text-3xl font-bold ${pColor(result.profitabilityPct)}`}>{result.profitabilityPct.toFixed(1)}%</p>
                  ...
                  <tbody className="divide-y">
                    {result.positions.map((pos, i) => (
                      <tr key={i} className={`hover:bg-gray-50 ${!pos.matchedRate ? "bg-orange-50" : ""}`}>
                        ...
                        <td className="px-4 py-3">
                          <div className="max-w-md">
                            <p className="text-gray-900 truncate" title={pos.description}>
                              {pos.description.slice(0, 80)}{pos.description.length > 80 ? "…" : ""}
                            </p>
                        ...
                        <td className="px-4 py-3 text-right">{fmt(pos.estimateCost)}</td>
                        <td className="px-4 py-3 text-right">{pos.matchedRate ? fmt(pos.contractorCost) : "—"}</td>
                        <td className="px-4 py-3 text-right font-medium">{pos.matchedRate ? fmt(pos.profit) : "—"}</td>
                        <td className={`px-4 py-3 text-right font-bold ${pos.matchedRate ? pColor(pos.profitabilityPct) : "text-gray-400"}`}>
            ...
            {result.positions.some((p) => !p.matchedRate) && (
              <div className="mt-4 p-4 bg-orange-50 border border-orange-200 rounded-lg">
                <p className="text-orange-800 text-sm">
                  ⚠️ Некоторые позиции не удалось сопоставить с расценками. Себестоимость по ним не учтена — реальная рентабельность может быть ниже.
                </p>
              </div>
            )}
          </>
